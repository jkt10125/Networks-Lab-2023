...
int showFile(char *content, int type, int len)
{
    int fd, cnt, i;
    char buffer[BUFLEN];
    char *args[4];
...
void processResponse(char *response, int sockfd)
{
    ...
    for (int i = 0; i + 3 < strlen(response); i++)
    {
        ...
        if (response[i] == '\r' && response[i + 1] == '\n' && response[i + 2] == '\r' && response[i + 3] == '\n')
        {
            ...
            content = (char *)malloc(sizeof(char) * (strlen(response) - i - 3));
            for (int j = 0; j + i + 4 <= strlen(response); j++)
    ...
    if (strlen(content) > 0)
    {
        if (len != -1)
        {
            ...
            // while (n < len)
            // {
            //     char *tmp = getResponse(sockfd);
            //     content = (char *)realloc(content, sizeof(char) * (n + 1 + strlen(tmp) + 1));
            //     for (int i = 0; i <= strlen(tmp); i++)
            //         content[++n] = tmp[i];
            // }
            // printf("len= %d, n=%d\n", len, n);
            // len-=2;
            int curr_byte = strlen(content) + 1;
            char buffer[BUFLEN];
            int recv_byte;
            while (1)
            {
        ...
        if (showFile(content, type, len) < 0)
        {
